...
def test_variant_key_is_order_independent():
    assert variant_key("custom_pipeline", {"a": 1, "b": 2}) == variant_key("custom_pipeline", {"b": 2, "a": 1})
...
def test_variant_key_distinguishes_config_and_adapter():
    ...
    assert variant_key("custom_pipeline", {}) != variant_key("simple", {})


def test_variant_key_none_config_equals_empty():
    assert variant_key("custom_pipeline", None) == variant_key("custom_pipeline", {})
    assert variant_key("custom_pipeline", {}).startswith("custom_pipeline@")
